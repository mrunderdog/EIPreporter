...
async function main(): Promise<void> {
  ...
  const output = {
    ...
    feedStatus: source.diagnostics.indexStatus ?? null,
    ...
    selectedPosts: view.selectedPosts.map((post) => ({
      ...
      KoreanSummary: post.summaryKo,
      interpretationState: post.interpretationState,
      lineage: post.evidenceParagraphIds,
      lineageCount: post.evidenceParagraphIds.length,
    })),
    articleFetchStatus: source.diagnostics.articleStatuses,
    parsedBodyLength: source.posts.map((post) => ({ title: post.title, length: post.cleanedText.length })),
